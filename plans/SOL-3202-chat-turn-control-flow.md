...
- We accept that a hung turn closes at the next interaction rather than within a minute, choosing lazy detection over a watchdog timer chain. The wedge a hung turn causes — queued messages, an open spinner — only matters when someone is looking, which is exactly when detection runs. Revisit if abandoned-tab hangs surface as user confusion.
...
- Post-ship signals: SSE connections per worker, ingest rejection rate (should be zero), stale-turn closes (should be rare), Redis memory against its cap, and Celery queue depth for any sign of broker keys being evicted.
...
- *Between 2 and 3* — this is where the new path gets proven. It is complete and live but uncalled, so drive `POST …/messages` against a test operation and watch a turn run end to end: the backend wakes and dispatches, the agent pushes, subscribers receive. That is the real production path, not a staging one. Confirm here too that the `HtmlEditSession` Lambda can reach Backend-Server's bundle route, VPC-attached or through the public ALB.
- *After 3* — watch for stale-turn closes, and leave `/stream` and the relay in place until they go quiet.
