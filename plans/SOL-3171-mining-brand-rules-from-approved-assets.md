...
| **Method 3 — Sampling with grounding verifier** (@ari) | Cheapest by an order of magnitude and flat in corpus size: 5 sampled assets to a rule-creator agent, then a rule-verifier agent checks grounding against 5 freshly sampled assets plus the originals (sample sizes scale down to the train-fold size on thin brands, reusing assets rather than crossing into eval). If this ties the others, the others don't ship. |
...
1. **Fold split.** Per brand, hold out ~20% of approved assets as the eval set. Miners see the train fold only. On thin brands (≤10 assets, Criterion 4) a flat 20% holdout leaves too few eval assets to anchor judging and too small a train fold for the fixed sample sizes below, so there we hold out a floor of 2–3 assets for eval, source the reference strip and all creator/verifier samples from the train fold, and let sampling reuse train assets rather than cross into the eval fold. *This corrects the brief as spiked, which ran the evaluator over all approved assets — including the ones the ruleset was mined from. Without the split we would be measuring memorisation.*
...
4. **Judging.** VLM sees A as anchor, plus B and C blind and position-randomised, alongside a reference strip of up to 5 approved assets — capped at what the corpus can spare on thin brands (see fold split). The strip is judge context only and is never mined, so it can be drawn from the train fold without leaking into the score. Forced choice, no ties, 3 votes per triple, majority wins. *Reading the brief's comparison as `sim(A,B) > sim(A,C)`: B vs. C is the contest, A is the anchor.*
