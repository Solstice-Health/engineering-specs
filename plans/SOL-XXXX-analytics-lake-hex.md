...
2. Is the `hex_ro` allowlist complete enough? It deliberately leaves out claims, clinical files, guideline results and Veeva documents. Which of those, if any, do analysts need, and in what content-free shape.
...
- Table-level access control, not UI hiding. `hex_ro` holds SELECT on an allowlist only: the metadata tables `companies`, `users`, `brands`, `brand_team_members`, `projects`, `admin_requests`, `marketing_files`, `prior_approved_files`, and two content-free views in a dedicated `analytics` schema, `assets` (n_cg_operations without prompt, messages, HTML, chat history, source material or metadata blobs) and `mlr_reviews` (qc results as counts and flags, no report payload). The onboarding script revokes every other grant and default privilege first and fails verification if a grant exists outside the list, so generated content and chat transcripts (`n_cg_operation_messages`, `chat_messages`, `n_cg_operation_html_versions`) cannot reach Hex or its LLM providers. Exposing a new table is a deliberate edit to the script. Hex schema filters are not relied on.
