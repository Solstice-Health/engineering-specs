...
2. Which tables and columns in tenant databases should never be reachable from Hex? The revoke list covers whole tables (chat bodies, HTML versions, prompt and template config). `n_cg_operations` still exposes `html_content`, `chat_history` and `messages` columns; should those become a view without them.
...
- Table-level access control, not UI hiding. `hex_ro` is granted SELECT on all tenant tables and then revoked on the tables that hold generated content bodies, chat transcripts, prompt and template configuration, and plumbing (`chat_messages`, `n_cg_operation_html_versions`, `prompt_registry`, `integrations`, the library tables, `veeva_annotations`, `claim_studio_sessions`, `brand_pipeline_overrides`, notification and timing tables). Hex schema filters mirror the same list for the agent's benefit but are not relied on for security. Default privileges are set per table owner so new tables are readable; a new sensitive table has to be added to the revoke list, which is a known maintenance step. Column-level exposure remains: `n_cg_operations` carries `html_content`, `chat_history` and `messages` columns; hiding those is open question 2.
...
| 2026-09-14 | Revoke sensitive tables from `hex_ro` in the database, not only in Hex's UI | Schema filters only; DB revoke | Automated review pointed out filters are not an ACL; the revoke list now lives in the onboarding SQL and the governance script mirrors it | agent, pending review | Proposed |
